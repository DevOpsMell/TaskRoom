...
describe('Column API', () => {
  ...
  test('POST /api/v1/columns should create a new column', async () => {
    ...
    const savedColumn = await columnModel.findById(
      response.body.message.split(' ')[0],
    );
    expect(savedColumn.name).toEqual(columnBody.name);
    expect(savedColumn.parent_project.toString()).toEqual(
      columnBody.parent_project,
    );
    await columnModel.findByIdAndDelete(savedColumn.id);
  });
  //POST Error
  ...
  test('PATCH /api/v1/columns/:id should return 400 for validation error', async () => {
    ...
    expect(response.body.message).toContain(
      'name: Path `name` (`1`) is shorter than the minimum allowed length',
    );
    await columnModel.findByIdAndDelete(column.id);
  });
